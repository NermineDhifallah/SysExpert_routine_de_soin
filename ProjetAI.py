    ...
    if x > 0:
        ...
        for ind, row in res.iterrows():
            ...
            card = Label(
                ...
                bg='white',
                fg='black',
                wraplength=200  # Adjust this value as needed
            )
            card.image = image  # Keep a reference to the image to prevent garbage collection
...
def update_question():
    ...
    if current_question in [3, 4]:
        for index, option_text in enumerate(options, start=1): 
            ...
            # Associer la fonction add_response à chaque Checkbutton
